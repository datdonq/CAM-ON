...
def capture_webcam(ip_camera_url, flag = False):
    if flag:
        ...
        while True:
            ...
            if not ret:
                break
            tracks, _ = process_frame(frame, model, tracker, class_names, colors)
...
async def detect(ip_camera_url, folder_path):
    cap = initialize_video_capture(ip_camera_url)
    model = initialize_model()
    class_names = load_class_names()
    ...
    while True:
        ...
        if not ret:
            break
        tracks, class_id = process_frame(frame, model, tracker, class_names, colors)
        if class_id == 1 : 
            frame = draw_tracks(frame, tracks, class_names, colors, class_counters, track_class_mapping)
            ...
            # Đợi 0.2 giây trước khi ghi frame tiếp theo
